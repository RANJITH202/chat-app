...
const ChatText = () => {
  ...
  const sendMessage = ({ message }) => {
    socket.emit("send_message", {message, senderId: userId, receiverId: participantsDetails._id});
  ...
  useEffect(() => {
    document.querySelector('.chat-text-container > div:last-child')?.scrollIntoView();
  }, [messages]);
  return (
    <>
      <div className="chat-text-container">
        {/* <ScrollToBottom> */}
          {messages.map((message) => (
            <div key={message._id} className={message.senderId === userId ? "chat-right" : "chat-left"}>
              {message.senderId === userId && <div className="sp"></div>}
              <div className="message">
                <p className="no-margin">{message.message}</p>
                <div className="check-time">
                  <span className="no-margin font-size-12">{moment(message.createdAt).format('hh:mm A')}</span>
                  {message.senderId === userId && ( message.isSeen ? (
